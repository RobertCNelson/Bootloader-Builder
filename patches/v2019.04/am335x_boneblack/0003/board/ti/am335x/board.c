...
const struct dpll_params *get_dpll_ddr_params(void)
{
	...
	else if (board_is_pb() || board_is_revolve() || board_is_bone_lt() || board_is_icev2() || board_is_beaglelogic() || board_is_blank_bone_lt())
...
void sdram_init(void)
{
	...
	else if (board_is_pb() || board_is_revolve() || board_is_bone_lt() || board_is_beaglelogic() || board_is_blank_bone_lt())
...
#ifdef CONFIG_ENV_VARS_UBOOT_RUNTIME_CONFIG
#ifdef CONFIG_TI_I2C_BOARD_DETECT
	if (!board_is_pb() && !board_is_beaglelogic() && !board_is_revolve()) {
		do_cape_detect();
	}
#endif
#endif
